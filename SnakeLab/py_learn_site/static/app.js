...
function updateSidebarProfileCard(){
  const acc = currentIdentity() || { username: 'guest', display: 'Guest', avatar: null };
  const av = document.getElementById('spc-avatar');
  const dn = document.getElementById('spc-display');
  const un = document.getElementById('spc-username');
  if(av){ av.src = acc.avatar || 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36"><rect width="100%" height="100%" fill="%239b7dff"/></svg>'; }
  if(dn){ dn.textContent = acc.display || acc.username || 'guest'; }
  if(un){ un.textContent = acc.username || 'guest'; }
  // bind actions once
  const toProfile = document.getElementById('spc-profile');
  if(toProfile && !toProfile._bound){
    toProfile._bound = true;
    toProfile.addEventListener('click', ()=>{
      ...
      const btn = document.querySelector('.sidebar nav button[data-section="profile"]') || document.querySelector('[data-target="profile"]'); if(btn){ btn.classList.add('active'); }
    });
  }
  const sw = document.getElementById('spc-switch');
  if(sw && !sw._bound){
    sw._bound = true;
    sw.addEventListener('click', ()=>{
      const list = loadAccounts(); if(!list.length){ showToast('No accounts yet'); return; }
      let i = currentAccountIndex(); i = (i+1) % list.length; localStorage.setItem(ACTIVE_ACCOUNT_KEY, String(i));
      renderAccounts(); // will also refresh card
      showToast(`Switched to ${list[i].display || list[i].username}`);
    });
  }
}
...
function changeTask(delta){
  const tasks = getLessons()[currentLessonIndex].tasks;
...
function setupSettings(){
  ...
  if(themeSel){ themeSel.value = getSavedTheme(); themeSel.addEventListener('change', ()=>{ applyTheme(themeSel.value); showToast('Theme applied'); }); }
